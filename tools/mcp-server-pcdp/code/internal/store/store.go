...
func templateSearchDirs() []string {
	...
	if dirExists("/etc/pcdp/templates") {
		dirs = append(dirs, "/etc/pcdp/templates")
	}
	if home, err := os.UserHomeDir(); err == nil {
		if d := filepath.Join(home, ".config", "pcdp", "templates"); dirExists(d) {
			dirs = append(dirs, d)
		}
	}
	if dirExists(".pcdp/templates") {
...
func hintsSearchDirs() []string {
	dirs := []string{"/usr/share/pcdp/hints"}
	if dirExists("/etc/pcdp/hints") {
		dirs = append(dirs, "/etc/pcdp/hints")
	}
	if home, err := os.UserHomeDir(); err == nil {
		if d := filepath.Join(home, ".config", "pcdp", "hints"); dirExists(d) {
			dirs = append(dirs, d)
		}
	}
	if dirExists(".pcdp/hints") {
		dirs = append(dirs, ".pcdp/hints")
	}
	return dirs
}
...
//
//	/usr/share/pcdp/templates/    (pcdp-templates package)
//	/etc/pcdp/templates/          (system admin overrides)
//	~/.config/pcdp/templates/     (user overrides)
//	./.pcdp/templates/            (project-local)
