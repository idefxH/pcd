...
ResourceURI := string
// Format: "pcdp://<type>/<name>"
// Types: templates, prompts, hints
// Examples:
//   pcdp://templates/cli-tool
//   pcdp://prompts/interview
//   pcdp://prompts/translator
//   pcdp://hints/cloud-native.go.go-libvirt
...
PRECONDITIONS:
  ...
  - uri matches "pcdp://<type>/<name>" where type is one of:
...
STEPS:
  ...
  3. Run all pcdp-lint rules (RULE-01 through RULE-14) against content.
     MECHANISM: identical rule set and logic to the pcdp-lint CLI;
...
POSTCONDITIONS:
  ...
  - Result is identical to running: pcdp-lint <file> on the same content.
...
- lint_content and lint_file produce identical output to pcdp-lint CLI
  for identical input.
...
- [observable]      lint_content result is identical to pcdp-lint CLI on same input
...
- [implementation]  rule execution order: RULE-01 through RULE-14, same as pcdp-lint
- [implementation]  resource URIs follow pcdp://<type>/<name> scheme exactly
...
WHEN:
  tool read_resource called with uri="pcdp://prompts/interview"
THEN:
  response contains uri="pcdp://prompts/interview"
...
GIVEN:
  content is a valid PCDP spec with all required sections
WHEN:
...
GIVEN:
  content is a PCDP spec missing the INVARIANTS section
WHEN:
...
GIVEN:
  content is any PCDP spec
  the same content is saved to disk as "test.md"
WHEN:
  tool lint_content called with that content
  AND pcdp-lint CLI run on "test.md"
THEN:
...
COMPONENT: implementation
  ...
  notes: >
    ...
    (rule engine, shared with pcdp-lint), internal/store (template,
    prompt, hints stores). Reuse lint rule logic from pcdp-lint if
...
COMPONENT: packaging
  files: mcp-server-pcdp.spec, debian/control, debian/changelog,
...
COMPONENT: tests
  ...
  notes: >
    ...
    No filesystem access. No network calls. No live pcdp-lint binary.
